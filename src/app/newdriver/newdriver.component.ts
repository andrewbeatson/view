...
export class NewdriverComponent implements OnInit {
  ...
  create() {
    ...
    if (format.test(this.surname)) {
      this.error('Please enter valid surname');
      return false;
    }
    if (
      this.email === '' ||
          ...
          if (!datas.length) {
            this.api
              ...
              .then(
                (data) => {
                  this.spinner.hide();
                  this.api.alerts('Success', 'User Created', 'success');
                  this.navCtrl.back();
                },
                (error) => {
                  this.error(error);
